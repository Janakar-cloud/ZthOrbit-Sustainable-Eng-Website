...
router.get("/", requireAuth(["admin", "editor", "viewer"]), async (req, res) => {
...
router.patch("/:id", requireAuth(["admin", "editor", "viewer"]), async (req, res) => {
...
// Mark all as read
router.patch("/read-all", requireAuth(["admin", "editor", "viewer"]), async (req, res) => {
  await Notification.updateMany({ userId: req.user!.id, isUnread: true }, { isUnread: false });
  res.status(204).send();
});

// Delete single notification
router.delete("/:id", requireAuth(["admin", "editor", "viewer"]), async (req, res) => {
...
router.delete("/", requireAuth(["admin", "editor", "viewer"]), async (req, res) => {
